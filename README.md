...
Minimal vim-inspired tiling window manager for X11.

VIWM uses a modal workflow:

- `INSERT` for normal desktop input (can be accesed with i if you are in normal mode)
- `NORMAL` for window-manager actions ( super + esc)
- `COMMAND` for vim-style commands such as `:q!`  (tipe : in Normal mode)
